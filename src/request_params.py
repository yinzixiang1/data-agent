...
REQUEST_PARAMS: list[dict[str, Any]] = [
    ...
    {
        "key": "enable_summarize",
        "type": "boolean",
        "default": False,
        "label": "启用结果总结",
        "hint": "执行完 SQL 后用 LLM 对结果进行自然语言总结",
        "group": "SQL 执行",
        "section": "flow",
        "scope": ["nl2sql", "hybrid"],
        "depends_on": {"key": "enable_execute", "value": True},
    ...
    {
        "key": "enable_empty_analysis",
        "type": "boolean",
        "default": True,
        "label": "空结果分析",
        "hint": "查询结果为空时，让 LLM 分析可能原因并给出建议",
        "group": "纠错增强",
        "section": "flow",
        "scope": ["nl2sql", "hybrid"],
    },
    ...
    {
        "key": "enable_result_check",
        "type": "boolean",
        "default": True,
        "label": "结果合理性检查",
        "hint": "SQL 执行后检查结果是否符合预期（如金额为负、日期异常等）",
        "group": "纠错增强",
        "section": "flow",
        "scope": ["nl2sql", "hybrid"],
    },
