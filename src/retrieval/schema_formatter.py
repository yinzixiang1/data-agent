...
12. 只有用户原话、已确认的会话上下文以及本轮提供的术语、Schema、关系、枚举和 Value 证据能够唯一确定事实表、指标、字段、关联路径、筛选值和时间口径时，才生成 SQL。
   只要存在多种合理解释、证据不足、证据冲突或无法唯一确定 SQL，必须输出一行 JSON：
   NEED_CLARIFY: {"question":"需要用户确认的问题","options":[{"label":"选项文案","value":"用于补充原问题的完整含义"}]}
   options 仅在证据中存在明确候选项时提供，最多 4 个；只是缺少开放式信息时输出空数组。候选检索排名、最高分、Few-shot、常见做法和默认习惯都不能替代用户确认，不得主动猜测。
13. 澄清问题提到候选表时，必须同时写出 Schema 中的完整物理表名，格式为“业务说明（database.table）”，不得只写业务名称或别名。
14. 金额汇总不得跨币种直接相加；未换汇时必须输出并按对应币种分组，已换汇时使用明确的目标币种口径。
15. 只输出用户要求的字段、指标和保证口径正确所必需的维度；不得自行增加会改变分组粒度的字段。
16. 明细查询默认 LIMIT 100；聚合查询不得自行添加 LIMIT，只有用户明确要求 Top N 或数量限制时才限制分组数量。"""
