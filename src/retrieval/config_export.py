...
def export_config(agent_id: int, output_path: str):
    ...
    with engine.connect() as conn:
        # 1. Agent 基础信息
        row = conn.execute(
            text("SELECT id, name, handle, token FROM da_agent WHERE id = :id"),
            {"id": agent_id},
        ).fetchone()
        if not row:
    ...
    logger.info(
        f"启动命令: CONFIG_SOURCE=local CONFIG_PROFILE={output_path} uvicorn app:app --port 9090"
    )


def main():
    parser = argparse.ArgumentParser(
        description="从 MySQL 导出 Agent 配置到本地 JSON 文件"
    )
